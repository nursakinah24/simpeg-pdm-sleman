<template>
  <div class="pt-4">
    <div class=" flex flex-col p-2 bg-white shadow rounded-lg mb-4 border-solid border-2">
          ...
          <table class="max-w-full leading-normal font-poppins">
            <thead>
              <tr>
                ...
                <th scope="col"
                  class="px-24 py-3 text-sm text-center text-gray-800 uppercase bg-white border-b border-gray-200">
                  Komentar
                </th>
              </tr>
            </thead>
            <tbody class=" bg-sky-50">
              <template v-if="performanceData.length > 0">
                <tr v-for="(item, index) in performanceData" :key="index" class="divide-x">
                  ...
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
                    <p class="text-gray-900 whitespace-no-wrap">{{ item.nilai_kerja }}</p>
                  </td>
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
                    <p class="text-gray-900 whitespace-no-wrap">{{ item.predikat }}</p>
                  </td>
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
              ...
              <template v-else-if="loading">
                <tr>
                  <td colspan="8" class="text-center py-4 text-gray-500 font-semibold">Loading</td>
                </tr>
              </template>
              <template v-else>
                <tr>
    ...
    <performanceVerify v-if="addVerify" @close="addVerify = false" :visible="addVerify" :perfData="selectedPerf"
      :id="id" />
  </div>
</template>
...
import performanceVerify from '../../../components/Form/Admin/Verify/performanceVerify.vue'
...
export default {
  ...
  setup(props) {
    ...
    const selectedPerf = ref({})

    const toogleVerify = (perfData) => {
      addVerify.value = !addVerify.value;
      selectedPerf.value = perfData;
    }

    watch(() => props.performanceData, (newValue) => {
      if (newValue) {
        loading.value = false;
      }
    }, { immediate: true });
    ...
    return {
      loading, addVerify, selectedPerf, toogleVerify
    };
  }
};
</script>
