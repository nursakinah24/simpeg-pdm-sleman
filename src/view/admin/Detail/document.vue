<template>
  <div class="pt-4">
          ...
          <table class="max-w-full leading-normal font-poppins">
            <thead>
              <tr>
                ...
                <th scope="col"
                  class="px-20 py-3 text-sm text-center text-gray-800 uppercase bg-white border-b border-gray-200">
                  ID / Nomor
                </th>
                <th scope="col"
                  class="px-20 py-3 text-sm text-center text-gray-800 uppercase bg-white border-b border-gray-200">
            ...
            <tbody class=" bg-sky-50">
              <template v-if="documentData.length > 0">
                <tr v-for="(item, index) in documentData" :key="index" class="divide-x">
                  ...
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
                    <p class="text-gray-900 whitespace-no-wrap">{{ item.jenis_dokumen }}</p>
                  </td>
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
                    <p class="text-gray-900 whitespace-no-wrap">{{ item.no_dokumen }}</p>
                  </td>
                  <td class="px-4 py-5 text-sm text-center border-b border-gray-200">
              ...
              <tr>
                <td colspan="7" class="text-center py-4 text-gray-500 font-semibold">Loading</td>
              </tr>
            </template> 
            <template v-else>
              <tr>
    ...
    <documentVerify v-if="addVerify" @close="addVerify = false" :visible="addVerify" :docData="selectedDoc" :id="id" />
  </div>
</template>
...
import documentVerify from '../../../components/Form/Admin/Verify/documentVerify.vue'
...
export default {
  ...
  setup(props) {
    ...
    watch(() => props.documentData, (newValue) => {
      if (newValue) {
        loading.value = false;
      }
    }, { immediate: true });
    ...
    return {
      loading, addVerify, selectedDoc, toogleVerify
    };
  }
};
</script>
